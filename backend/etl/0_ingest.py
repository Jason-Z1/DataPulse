...
def discover_files(raw_root: Path):
    # Find all TXT, CSV, PARQUET files and ZIP archives in all interval subfolders, recursively.
...
def main():
    ...
    # Debug: confirm discovery output
    print("RAW_ROOT:", RAW_ROOT)
    print("items discovered:", len(items))
    # Optionally: uncomment to see a sample
    # for it in items[:20]:
    #     print("ITEM:", it)
    ...
    if not items:
        logger.warning("No TXT/CSV/PARQUET or ZIP files found under RAW_ROOT!")
        return
